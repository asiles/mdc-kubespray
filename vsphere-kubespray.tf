...
# HAProxy server backend template Masters #
data "template_file" "haproxy_backend_master" {
  count    = "${length(var.vm_master_ips)}"
  template = "${file("templates/haproxy_backend_master.tpl")}"
...
data "template_file" "haproxy_frontend_http" {
  template = "${file("templates/haproxy_frontend_http.tpl")}"
...
# HAProxy server backend HTTP #
data "template_file" "haproxy_backend_apphttp" {
  count    = "${length(var.vm_master_ips)}"
  template = "${file("templates/haproxy_backend_apphttp.tpl")}"
...
data "template_file" "haproxy_frontend_https" {
  template = "${file("templates/haproxy_frontend_https.tpl")}"
...
# HAProxy server backend HTTP #
data "template_file" "haproxy_backend_apphttps" {
  count    = "${length(var.vm_master_ips)}"
  template = "${file("templates/haproxy_backend_apphttps.tpl")}"
...
resource "null_resource" "kubespray_download" {
  provisioner "local-exec" {
    command = "cd ansible && rm -rf mdc-kubespray && git clone ${var.k8s_kubespray_url}"
  }
}
...
resource "null_resource" "kubespray_create" {
  ...
  provisioner "local-exec" {
    command = "cd ansible/mdc-kubespray && ansible-playbook -i ../../config/hosts.ini -b -u ${var.vm_user} -e 'ansible_ssh_pass=${var.vm_password} ansible_become_pass=${var.vm_privilege_password} kube_version=${var.k8s_version}' ${lookup(local.extra_args, var.vm_distro)} -v cluster.yml"
  }
